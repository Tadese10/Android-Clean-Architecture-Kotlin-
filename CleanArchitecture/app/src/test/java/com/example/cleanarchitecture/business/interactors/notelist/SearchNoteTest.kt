...
import com.example.cleanarchitecture.business.data.cache.FORCE_SEARCH_NOTES_EXCEPTION
...
import com.example.cleanarchitecture.business.interactors.notelist.SearchNotes.Companion.SEARCH_NOTES_NO_MATCHING_RESULTS
import com.example.cleanarchitecture.business.interactors.notelist.SearchNotes.Companion.SEARCH_NOTES_SUCCESS
import com.example.cleanarchitecture.framework.datasource.cache.database.ORDER_BY_ASC_DATE_UPDATED
...
class SearchNoteTest{
    ...
    @Test
     fun blankQuery_success_confirmNotesRetrieved() = runBlocking {
        ...
        //Confirm that the notes were retrieved from the cached
        assertTrue{
            results != null
        }

        //Confirm the notes in the cached match with the notes retrieved
        val notesInTheCached = noteCacheDataSource.searchNotes(
            query = query,
            filterAndOrder = ORDER_BY_ASC_DATE_UPDATED,
            page = 1
        )
        ...
        assertTrue{
            results?.containsAll(notesInTheCached) ?: false
        }
    }
