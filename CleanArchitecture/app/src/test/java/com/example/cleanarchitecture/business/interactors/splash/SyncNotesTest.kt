...
class SyncNotesTest {
    ...
    fun doSuccessiveUpdatesOccur() = runBlocking {
        ...
        val updatedNote = Note(
            ...
            createdAt = noteNetworkDataSource.getAllNotes().get(0).createdAt,
            updatedAt = newDate
        )
        ...
        for(note in notes){
            if(note.id.equals(updatedNote.id)){
                assertTrue { note.updatedAt.equals(newDate) }
            }
        }
    }
        ...
        for(note in notes){
            noteCacheDataSource.searchNoteById(note.id)?.let { n ->
                ...
                if(n.id.equals(updatedNote.id)){
                    assertTrue { n.updatedAt.equals(newDate) }
                }
                else{
                    assertFalse { n.updatedAt.equals(newDate) }
                }
            }
        }
    }
