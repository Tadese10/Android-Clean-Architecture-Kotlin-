...
import com.example.cleanarchitecture.business.data.cache.CacheErrors
import com.example.cleanarchitecture.business.data.cache.FORCE_GENERAL_FAILURE
import com.example.cleanarchitecture.business.data.cache.FORCE_NEW_NOTE_EXCEPTION
...
import com.example.cleanarchitecture.business.interactors.notelist.InsertNewNote.Companion.INSERT_NOTE_FAILED
import com.example.cleanarchitecture.business.interactors.notelist.InsertNewNote.Companion.INSERT_NOTE_SUCESS
...
class InsertNewNoteTest {
    //System in test
    // system in test
    private val insertNewNote: InsertNewNote //System we are testing is InsertNewNote functionality
    ...
    fun insertNote_success_confirmNetworkAndCacheUpdated() = runBlocking {
        ...
        val cacheNoteThatWasUpdated = noteCacheDataSource.searchNoteById(newNote.id)
        assertTrue{
            cacheNoteThatWasUpdated == newNote
        }
    ...
    fun insertNote_fail_confirmNetworkAndCacheUnchanged()= runBlocking {
        val newNote = noteFactory.createSingleNote(
            id = FORCE_GENERAL_FAILURE,//We are forcing it to fail
            title = UUID.randomUUID().toString()
        )
    ...
    fun throwException_checkGenericError_confirmNetworkAndCacheUnchanged() = runBlocking {
        val newNote = noteFactory.createSingleNote(
            id = FORCE_NEW_NOTE_EXCEPTION,//We are forcing it to fail
            title = UUID.randomUUID().toString()
        )
